...
<body>
  <div class="container">
    <h1>Zo Computer API Gateway</h1>
    <p class="subtitle">Anthropic Messages API 兼容网关 — 用你自己的 Zo Access Token 访问 Claude</p>

    <div class="card">
      <p>这个网关提供兼容 <code>/v1/messages</code> 的接口。你可以注册 Zo Computer，获取 Access Token，然后把下面的 Base URL 配到客户端里直接使用。</p>
    ...
    <ol class="steps">
      ...
      <li>
        <h3>调用网关</h3>
        <p>把 Token 放进 <code>Authorization: Bearer</code> 或 <code>x-api-key</code> 请求头，目标地址使用上面的 Base URL。</p>
      </li>
    </ol>
  ...
  -d '{
    ...
    <h2>客户端配置</h2>
    <div class="card">
      <p>在支持自定义 Base URL 的 Anthropic 客户端中，将 Base URL 设置为：</p>
      <div class="base-url">${baseUrl}</div>
      <p>API Key 填写你的 Zo Access Token 即可。</p>
    </div>
